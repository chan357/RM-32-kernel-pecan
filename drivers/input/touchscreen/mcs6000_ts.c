...
//	unsigned char z_buf[1];
	...
	pCommand = MCS6000_TS_INPUT_INFO;
...
touch_retry:
	if (dev->pendown) {
		...
		when_resuming--;
		queue_delayed_work(dev->ts_wq, &dev->work,msecs_to_jiffies((when_resuming>0) ? 0 :  TS_POLLING_TIME));
	} else {
...
static int mcs6000_ts_off(void)
{
...
void mcs6000_firmware_info(unsigned char* fw_ver, unsigned char* hw_ver)
{
	...
	msleep(200);
	/* mcs7000 need STOP signal after writing addres by bongkyu.kim */
	i2c_smbus_write_byte(dev->client, MCS6000_TS_FW_VERSION);
	data = i2c_smbus_read_byte(dev->client);
	printk(KERN_INFO "MCS6000 F/W Version [0x%x]\n", data);
	dev->input_dev->id.version = data;

	i2c_smbus_write_byte(dev->client, MCS6000_TS_HW_REVISION);
	data = i2c_smbus_read_byte(dev->client);
...
{	
	...
	is_touch_suspend = 0;
	when_resuming = TS_POLLING_BOUNCE;
}
#endif
